...
| HTTPX worker | Screenshot identity comes from the main document's Chromium response and address-bar URL at capture. HTTPX probe results remain separate `probe_*` details. Observed redirect destinations emit bound `url` events. |
...
HTTPX's HTTP probe and browser screenshot are separate operations. The worker now captures the image directly through Chromium's debugging API and records its main-document response. A screenshot's typed target and `value` describe the final URL/IP/port, with `info.endpoint_evidence=browser_document` and `browser_endpoint_evidence=cdp_response`. The original URL/endpoint remains in `input`. `info.initial_url`, `initial_host`, `final_url`, and `response_url` distinguish the initial task, final address-bar state, and actual document response. Fragments and same-document history changes can make the last two differ. `info.redirect_chain` is a JSON string containing main-document responses in order (`url`, `host`, `port`, `status_code`, and optional `location`, `content_type`, `server`). Subframes and subresources are excluded.

Screenshot bytes use `info.screenshot_status=captured`; failed attempts can retain a response/chain with `missing` and no bytes. HTTPX's independently observed metadata is namespaced under `probe_*` (including `probe_url` and `probe_host`). It must not be merged onto a different final endpoint. Observed redirect destinations emit separate bound `url` events with `probe=browser-redirect`; the SDK supplies lineage and gates. `meta.redirect_depth` carries the cumulative navigation budget across generated URL tasks.

The default profile configures a task-local DNS override and browser hostname mapping for bound inputs. `info.endpoint_binding=task_dns_and_browser_rule`, `binding_name`, and `binding_host` describe that input binding, not every destination in the chain. `info.dns_evidence=task_override` means synthetic routing answers were used; HTTPX A/AAAA/CNAME enrichment from that invocation is omitted. `info.artifact_dir` locates isolated local artifacts. Previously exported screenshots with `endpoint_evidence=http_probe` or `browser_endpoint_evidence=unreported` retain their original, weaker evidence; readers must not reinterpret them as browser observations.

Nuclei findings use `info.endpoint_evidence=scanner_reported`; IP and matched location come from the scanner output, which may not describe the final HTTP response after redirects. Bound HTTP and TCP/TLS scans record `info.endpoint_binding=task_dns`. `scan_requests` and `scan_request_errors` summarize the invocation's request trace; service scans also include `scan_off_endpoint_requests`. A task can fail with valid findings retained. Unexpected service endpoints are labeled `requested_endpoint_status=mismatch`; they never replace the actual observed identity with the requested one. Explicit `matcher-status=false` records and records containing an execution `error` are not findings. TCP/SSL observations use the reported endpoint and TCP transport, without inheriting an ancestor's name, port, SNI, or UDP identity. DNS findings from a bound resolver invocation are suppressed because those answers can be synthetic.

TLSX reports unsupported UDP inputs as terminal failures before launching its TCP probe; its results do not assert DTLS coverage. Findings from templates without reliable endpoint metadata retain only the known subject fields and the original `input`.
