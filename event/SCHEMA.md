...
The default HTTPX profile now configures a task-local DNS override and browser hostname mapping for bound inputs. `info.endpoint_binding=task_dns_and_browser_rule` records that configuration, not a final-browser observation; `info.browser_endpoint_evidence=unreported` makes the remaining limit explicit. `info.dns_evidence=task_override` means synthetic routing answers were used; A/AAAA/CNAME enrichment from that invocation is omitted. `info.artifact_dir` locates the invocation's isolated local artifacts, not a durable object-store reference.
