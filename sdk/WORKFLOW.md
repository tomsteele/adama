...
This is task coverage of observed inputs, not proof that every possible hostname, port, or vulnerability exists in the inventory. It does not provide a sealed run lifecycle or port-range/UDP uncertainty accounting. Browser observations describe the document at capture time.
...
This closes scheduling gaps for known name/address/open-port combinations. The HTTPX worker binds the initial hostname to the input IP in its probe resolver and browser mapping, with arguments kept in YAML. Chromium's main-document responses establish the screenshot endpoint; redirected screenshots use the final URL/IP/port, while input lineage and `info.redirect_chain` retain the original request and observed responses. Probe details remain under `probe_*`. A missing image, missing peer evidence, wrong initial backend, or a navigation change during capture fails the task while preserving observed responses. Missing binding configuration pauses the tool.

Observed redirect destinations return to the bus as bound `url` events. The SDK inherits gates and lineage normally. `meta.redirect_depth` preserves the browser profile's cumulative `max_redirects` budget across new URL tasks, so a chain cannot repeatedly create a fresh full navigation budget. This adds no central routing table or worker registration list.
